...
describe("SettingsView connector devices nav", () => {
  ...
  it("hides Devices nav when connector is disabled", async () => {
    ...
    await waitFor(() => {
      ...
      expect(within(nav).getAllByText("Overview").length).toBeGreaterThan(0);
    });
    const nav = screen.getByRole("navigation", { name: /Settings sections/i });
    expect(within(nav).queryAllByText("Devices")).toHaveLength(0);
  });
  ...
  it("shows Devices nav when connector is enabled", async () => {
    ...
    await waitFor(() => {
      const nav = screen.getByRole("navigation", { name: /Settings sections/i });
      expect(within(nav).getAllByText("Devices").length).toBeGreaterThan(0);
    });
  });
